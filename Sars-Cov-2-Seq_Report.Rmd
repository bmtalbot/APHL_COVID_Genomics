...
f <- as.data.frame(table(Current_Run$collection_date))
...
ggplot(data = Current_Run, aes(x=collection_date)) + geom_histogram(aes(x = collection_date)) +
      ...
      geom_vline(aes(xintercept = Current_Run$seq_date), linetype = "dashed") +
      geom_text(aes(x=Current_Run$seq_date, y = max(f$Freq),label="Sequencing \nDate",hjust=1, family = "serif"), size = 4.5) +
