...
runresults <- runresults %>% mutate(raw_reads1 =  if_else(!(fastqc_raw %in% .), fastqc_raw, fastqc_raw1)) %>% mutate(rawreads2 = fastqc_raw_reads_2)
...
runresults2 <- runresults %>% filter(!(is.na(collection_date)))  %>%
  ...
  mutate(Assembly_Count_Fail = case_when(assembly_status %in% "FAIL" ~ 1, assembly_status %in% "PASS" ~ 0)) %>%
      mutate(percent_SC2_reads = case_when(kracken_sc_2 == 'TBD' ~ NA_real_, TRUE ~ as.numeric(kracken_sc_2))) %>%
      mutate(percent_human_reads = case_when(kracken_human == 'TBD' ~ NA_real_, TRUE ~ as.numeric(kracken_human)))
