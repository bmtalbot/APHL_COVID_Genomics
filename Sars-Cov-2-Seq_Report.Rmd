...
runresults <- read.csv("batches_2001-2161.run-results_withBatch_link_blinded2.0.csv")
...
ggplot(data = Current_Run, aes(x=collection_date)) + geom_histogram(aes(x = collection_date)) +
      ...
      theme_classic() +
      theme(plot.title = element_text(hjust=0.5), axis.title.x = element_text(size = 14), axis.title.y = element_text(size=14))
```
...
Current_Run_Controls <- Current_Run %>% filter(seq_date==max(seq_date)) %>% filter(is.na(collection_date)) %>% select(sample_id, num_ACTG, Limit, Control_Status)
...
# ggplot(tail(clean_batchsummary_table,n=8), aes(x=batch_id, y = Per_Passing, fill=batch_id)) +
#   geom_point(aes(x=batch_id, y=Per_Passing, group=1), color = "red") + geom_text(aes(label=Per_Passing), vjust = -0.5) +
#   geom_path(aes(x=batch_id, y=Per_Passing, group=1), color = "red") +
#   geom_his(aes(x=batch_id, y=1*Freq, fill=)) +
#   scale_fill_manual(values=c("gray78","gray78","gray78", "gray78","gray78","gray78","gray78","blue")) +
#    ylab("Number of Sequences in Batch") +
#   xlab("Batch ID") +
#   ggtitle("Number and Percent of Passing Assemblies") +
#     #scale_y_continuous(sec.axis = sec_axis(~./1, name = "Number of Passing Sequences")) +
...
ggplot(tail(Assemblies, n=16), aes(x=batch_id, y=Percent, fill=assembly_status, color=batch_id)) +
  geom_point(aes(x=batch_id, y=Percent, group=1), color = "red") + geom_text(aes(label=Percent), vjust = -0.5) +
  geom_path(aes(x=batch_id, y=Percent, group=1), color = "red") +
  geom_bar(position="stack", stat="identity", size = 2) + 
  scale_fill_manual(values=c(cols)) +
  scale_color_manual(values=c(rep("white", times=8),"black")) +
  ylab("Percent of Sequences in Batch") +
  xlab("Batch ID") +
  ggtitle("Assemblies Status of Sequences") +
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5), axis.text.x=element_text(color=c(rep("black",times=8),"blue"),face="bold")) + 
...
rawplot <- ggplot(lastfive, aes(x=batch_id, y=fastqc_raw_reads_1)) + 
  ...
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5, size = 10), axis.text.x = element_text(size = 8, face = "bold"), axis.title.x = element_text(size = 8), axis.title.y = element_text(size = 8))
