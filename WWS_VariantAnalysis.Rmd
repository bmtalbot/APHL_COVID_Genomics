...
# Data Source
These data are generated from single site wastewater samples which were sequenced for evidence of SARS-CoV-2. Abundances of viral lineages were calculated using Freyja. 
...
```{r cleaner Lineage plot at threshold, echo=FALSE , warning=FALSE, message=FALSE, results = "asis", type = 'latex', fig.height=10, fig.width=12}
...
```
## Lineage Abundance Across Sample Set 

```{r, echo=false}
## First pass, see if there are any lianges within all of the major lineages that have a higher frequency that 1% in each sequence

library(dplyr)
sumtable <- all%>% dplyr::group_by(LineageClean) %>% dplyr::summarise(count = n(), lineage_mean = 100*mean(Frequency), Upper_SE = 100*((mean(Frequency) + sd(Frequency)/sqrt(length((Frequency))))), Lower_SE = 100*((mean(Frequency) - sd(Frequency)/sqrt(length((Frequency))))), maxium = 100*max(Frequency))

sumtable <- sumtable[order(-sumtable$lineage_mean),]

knitr::kable(sumtable, "html", col.names = c("Lineage", "Samples", "Mean Abundance (%)", "Upper SE ", "Lower SE", "Max Abundance"), align = "c", digits = 2)

```

## Variants of Concern, Interest, and Being Monitored ##
```{r, include=FALSE, echo=FALSE , warning=FALSE, message=FALSE}
